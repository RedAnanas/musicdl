...
4. 由用户确认后再提交、合并至 `develop`，并在 `develop` 上完成对应验证。
5. 仅在用户再次明确确认后，才可将 `develop` 合并并推送至 `master`。
6. 确认功能分支已被 `develop` 包含且验证通过后，删除本地 `feature/*`、`fix/*` 或 `chore/*` 源分支；远程同名分支仅在 PR 已合并或不再需要协作时删除。
